...
# 定义 SigmoidLoss 类，继承自自定义的 Loss 类，用于计算损失
class SigmoidLoss(Loss):

	# 初始化函数，接受可选的对抗温度（adv_temperature）
	def __init__(self, adv_temperature = None):
		super(SigmoidLoss, self).__init__()  # 调用父类的初始化方法
		# 使用 LogSigmoid 作为损失计算的基础
	...
	def forward(self, p_score, n_score):
		...
		if self.adv_flag:
			# 计算正样本的 LogSigmoid 损失，负样本部分的损失根据权重加权求和
